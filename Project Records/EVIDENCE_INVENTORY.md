...
the first preservation commit. Its file names are historical identifiers: legacy configurations are now in `../Cooja Configurations/Legacy Root Configurations/`, source snapshots and old root evidence are in `../Development Archive/06 Legacy Root Evidence/`, and this manifest itself is in `Project Records/`. Classification key:
