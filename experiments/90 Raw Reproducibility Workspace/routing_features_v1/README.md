...
The results directory also contains a seed-separated online drift-monitoring evaluation. A one-sided CUSUM is calibrated on blackhole rank-state windows and then applied to each sinkhole run in time order. Its inputs are generic RPL low-rank state features; labels and attack markers are used only for evaluation.
