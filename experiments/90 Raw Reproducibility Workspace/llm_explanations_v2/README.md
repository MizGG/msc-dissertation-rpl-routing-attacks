...
python3 scripts/score_llm_explanations_v2.py \
  --outputs experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/evaluation/handcrafted_schema_fixture.jsonl
python3 scripts/generate_llm_explanation_fixture_v2.py
python3 scripts/score_llm_explanations_v2.py \
  --outputs experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/outputs/deterministic_explanations.jsonl \
  --out-dir experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/evaluation/scored_deterministic
```

The handcrafted fixture and deterministic explanation score are software/demo
checks only. They validate schema and evidence handling, not external live-LLM performance.
The prompts use opaque case identifiers and do not expose attack-family labels.

## Local Offline Demonstration

Install and run a local model once. No API key is used, and the runner calls only
`http://127.0.0.1:11434` on this computer.

```sh
brew install --cask ollama
ollama pull qwen2.5:3b
python3 scripts/run_local_ollama_explanations.py --model qwen2.5:3b --limit 1
python3 scripts/score_llm_explanations_v2.py \
  --outputs "experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/outputs/ollama_explanations.jsonl" \
  --allow-subset \
  --out-dir "experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/evaluation/scored_ollama_demo"
```

For the presentation, run one selected case. Full nine-case batches are suitable
for offline evaluation but take longer on a laptop.
...
The automatic scorer is a rule-based screening check for schema completeness, evidence references, uncertainty language, drift context, actionability and explicitly unsupported claims. It is not a substitute for blinded human evaluation or a semantic factuality benchmark. Deterministic fixture scores must not be reported as live-model results.
