...
A follow-up window-level feature extractor was then added. Each 540-second simulation is represented as nine 60-second windows, with attack windows labelled positive only after the 240-second activation time. This improves the temporal framing of the experiment and avoids treating pre-activation attack-run behaviour as malicious. The blackhole window model performs strongly under leave-seed validation, but still fails to detect sinkhole windows when trained only on blackhole. With three sinkhole adaptation seeds, held-out sinkhole recall reaches 1.00, but precision remains low. This indicates partial recovery with a false-positive cost and confirms that stronger routing-specific features are still required.

The supplied Gope dataset was then audited to reconnect the Cooja work to the source research baseline. The audit found 768,811 rows across eight attack CSV files. Seven files contain 71 columns and a `TYPE` label; Worst Parent contains 46 columns and no `TYPE` label. The labelled files include routing-aware features such as rank, parent information, DIO/DAO/DIS counts, hop count and packet loss. These fields support the methodological conclusion from the Cooja experiments: sinkhole detection requires routing-state evidence, not only application throughput or radio-volume summaries.
