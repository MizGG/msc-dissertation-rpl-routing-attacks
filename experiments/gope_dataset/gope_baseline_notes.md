...
The model uses a simple Gaussian baseline over routing-aware features such as rank, parent count, DIO/DAO/DIS counts, hop count and packet loss. Rows are class-balanced per attack family before training. Non-numeric categorical fallback values use a deterministic BLAKE2b mapping, not Python's process-randomised hash.
