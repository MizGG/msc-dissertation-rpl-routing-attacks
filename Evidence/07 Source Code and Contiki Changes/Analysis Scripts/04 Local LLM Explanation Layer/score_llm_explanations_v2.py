...
def score(output: dict[str, object], rubric: dict[str, object]) -> dict[str, object]:
    ...
    uncertainty = 1 if any(word in text for word in UNCERTAINTY) else 0
    drift_alignment = 2 if any(word in text for word in DRIFT) else 0
    actionability = 1 if any(word in str(response.get("recommended_action", "")).lower() for word in ACTION) else 0
    schema = 1 if not missing and str(response.get("confidence", "")).lower() in {"low", "medium", "high"} else 0
    limitations = 1 if str(response.get("limitations", "")).strip() else 0
    unsupported = sorted(phrase for phrase in FORBIDDEN if phrase in text)
    raw_total = evidence_fidelity + mechanism_alignment + uncertainty + drift_alignment + actionability + schema + limitations
...
def main() -> None:
    ...
    parser.add_argument(
        "--rubric",
        type=Path,
        default=Path("experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/evaluation/evaluation_cases.jsonl"),
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("experiments/90 Raw Reproducibility Workspace/llm_explanations_v2/evaluation/scored"),
    )
    parser.add_argument("--allow-subset", action="store_true", help="Score a selected subset of rubric cases")
    args = parser.parse_args()

    outputs = read_jsonl(args.outputs)
    rubrics = {str(row["alert_id"]): row for row in read_jsonl(args.rubric)}
    output_ids = {str(row.get("alert_id")) for row in outputs}
    if not output_ids:
        raise ValueError("No output cases supplied")
    if not output_ids.issubset(rubrics):
        raise ValueError("Output contains alert IDs absent from evaluation cases")
    if not args.allow_subset and output_ids != set(rubrics):
        raise ValueError("Output alert IDs do not exactly match evaluation cases; use --allow-subset for a selected demo case")
