...
DEFAULT_FEATURES = Path("experiments/90 Raw Reproducibility Workspace/trust_layer_v1/features/trust_routing_window_features.csv")
DEFAULT_DRIFT = Path("experiments/90 Raw Reproducibility Workspace/cross_attack_drift_with_sybil_v1/results/cross_attack_matrix.csv")
DEFAULT_ADAPT = Path("experiments/90 Raw Reproducibility Workspace/cross_attack_adaptation_with_sybil_v1/cross_attack_adaptation_summary.csv")
DEFAULT_OUT = Path("experiments/90 Raw Reproducibility Workspace/llm_explanations_v2")

FEATURE_DISPLAY_NAMES = {
    "attacker_node16_radio_tx": "monitored_node_radio_tx",
}
...
def feature_snapshot(observed: dict[str, str], reference: dict[str, str]) -> dict[str, object]:
    ...
    for field in CASE_FEATURES:
        ...
        output[FEATURE_DISPLAY_NAMES.get(field, field)] = {
            "observed": obs,
            "matched_control": ref,
...
def main() -> None:
    ...
    for case_number, family in enumerate(families, start=1):
        ...
        alert_id = f"rpl-case-{case_number:02d}"
        evidence = {
            "alert_id": alert_id,
