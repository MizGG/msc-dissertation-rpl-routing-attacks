...
RUN_RE = re.compile(r"(?:(?P<condition>ATTACKER_RELOCATED|LOSSY_RADIO)_)?(?P<family>BH|SH|DIS_FLOOD|GRAYHOLE|INCREASE_RANK|DIO_SUPPRESSION|WORST_PARENT|WORMHOLE|SYBIL)_(?P<mode>ATTACK|CONTROL|DEFENCE)_N(?P<nodes>\d+)_SEED(?P<seed>\d+)")
...
def empty_window(run_id: str, family: str, mode: str, seed: str, nodes: str, start: int, width: int) -> dict[str, object]:
    return {
        ...
        "family": family,
        "mode": mode.lower(),
        "binary_run_label": 0 if mode == "CONTROL" else 1,
        "window_label": 1 if mode != "CONTROL" and start >= 240 else 0,
        "nodes": int(nodes),
        "seed": seed,
...
def parse_run(run_dir: Path, width: int, duration: int, root_node: int) -> list[dict[str, object]]:
    ...
    condition = match.group("condition") or "BASELINE"
    for row in windows:
...
def main() -> None:
    ...
    parser.add_argument("--runs-dir", type=Path, default=Path("experiments/90 Raw Reproducibility Workspace/routing_features_v1/runs"))
    parser.add_argument("--out", type=Path, default=Path("experiments/90 Raw Reproducibility Workspace/routing_features_v1/features/routing_window_features.csv"))
